...
public class window extends JFrame implements ActionListener {
	...
	private JButton btnMachineFirst;
	...
	public window() {
		...
		btnMachineFirst = new JButton("machine first");
		btnMachineFirst.setBounds(335, 126, 118, 25);
		btnMachineFirst.addActionListener(this);
		contentPane.add(btnMachineFirst);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btnMachineFirst) {
			machineFirst();
		}
	}
	...
	class LabelAdapter extends MouseAdapter {
		...
		public void mouseReleased(MouseEvent me) {
			int GO = TTT.checkEnd();
			if (TTT.userMove(sender.Field - 1) && GO == 0) {
				sender.setForeground(Color.GREEN);
				sender.setText("X");
				ComputerMove(-1, Color.RED);
				if ((GO = TTT.checkEnd()) != 0)
					SetCounters(GO);
			}
		}
	}
	...
	private void ComputerMove(int player, Color pcolor) {
		int move; 
		if(TTT.checkEnd() != 0)
			move = 0;
		else
			move = TTT.machineMove() + 1;
